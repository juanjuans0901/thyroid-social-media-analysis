...
# ── Fonts ─────────────────────────────────────────────────────────────────────
# This figure contains no Chinese glyphs, so it is set entirely in a
# metric-compatible Arial substitute (Liberation Sans), matching Figure 1 and
# the manuscript body text. This also makes bold weights available: the Noto
# Sans CJK SC package ships a Regular face only, so setting it as the primary
# family silently discards every fontweight="bold" request.
...
# Global type scale. Figure 4 carries a great deal of text, so it is set one
# step larger to survive reduction to journal column width.
FS = 1.20

# ── Unified palette, anchored to Figures 2, S3, S4 and S6 ─────────────────────
C = {"T1": "#1f77b4", "T2": "#d2691e", "T3": "#2ca02c",
     "T4": "#9467bd", "T5": "#17becf"}
GREY = "#0d0d0d"          # darkened per author restyle
GREY_NOTE = "#262626"     # Panel B italic explanatory notes
...
T(50, 97.8, "Figure 4. Thematic Landscape of Thyroid Nodule and Cancer Patient "
  ...
  ha="center", fontsize=17.4, fontweight="bold")
...
for i, (k, name, n, csm) in enumerate(TOP):
    ...
    T(x + wid / 2, 90.9, k, ha="center", fontsize=14.6, fontweight="bold", color=C[k])
    T(x + wid / 2, 89.6, name, ha="center", va="top", fontsize=11.0, fontweight="bold")
    yy = 87.3 if "\n" in name else 88.4
    T(x + wid / 2, yy, n, ha="center", va="top", fontsize=10.5, style="italic", color="#3d4852")
    ax.add_patch(Rectangle((x, 85.0), wid, 1.5, fc=C[k], alpha=0.13, ec="none"))
    T(x + wid / 2, 85.75, csm, ha="center", va="center", fontsize=10.5,
      fontweight="bold", color=C[k])

# ══════════════════════════════════════════════════════════ Panel B
ax.add_line(plt.Line2D([2, 98], [83.4, 83.4], color="#c9ced6", lw=1.1))
ax.add_patch(FancyBboxPatch((2.2, 67.9), 95.6, 14.8,
...
T(3.4, 80.5, "The coding frame was developed alongside rather than after the final "
  ...
  fontsize=10.0, style="italic", color=GREY_NOTE)
T(3.4, 79.5, "domain identifiers D1–D5 are independent of topic identifiers T1–T5. "
  ...
  fontsize=10.0, style="italic", color=GREY_NOTE)
T(3.4, 78.5, "Full definitions in Supplemental Table S7.",
  fontsize=10.0, style="italic", color=GREY_NOTE)
...
pos = [(4.6, 76.3), (23.6, 76.3), (42.6, 76.3), (61.6, 76.3), (80.6, 76.3)]
for (dx, dy), (dname, cats) in zip(pos, DOM):
    T(dx, dy, dname, fontsize=10.9, fontweight="bold", color=GREY)
    ax.add_line(plt.Line2D([dx, dx + 17.4], [dy - 0.55, dy - 0.55],
                           color="#aeb5bf", lw=0.9))
    for j, c in enumerate(cats):
        T(dx + 0.35, dy - 1.95 - j * 1.25, "•  " + c, fontsize=9.6)
...
for lab, col in leg:
    ...
    T(lx + 2.9, 65.0, lab, va="center", fontsize=11.2)
    lx += len(lab) * 0.55 + 6.0

BL, BW = 13.0, 80.0
y = 62.7
for k in ["T1", "T2", "T3", "T4", "T5"]:
    T(4.2, y - 1.15, k, fontsize=12.3, fontweight="bold", color=C[k], va="center")
    for r, (row, lbl) in enumerate(zip(SENT[k], ["Posts", "Comments"])):
        yy = y - r * 2.3
        T(12.3, yy - 0.5, lbl, ha="right", va="center", fontsize=9.9, color=GREY)
        cx = BL
...
T(50, 36.1, "Solid bars = posts ($\\it{N}$ = 1,916); lighter bars = comments "
  ...
  ha="center", fontsize=11.2, color="#3d4852")
...
T(50, 30.4, "C O G N I T I V E   R E P R E S E N T A T I O N S",
  ha="center", fontsize=11.2, fontweight="bold", color=GREY)
cw = 18.2
for i, (nm, q, desc, arrow, col) in enumerate(CSM):
    x = 3.0 + i * (cw + 0.5)
    ax.add_line(plt.Line2D([x, x + cw], [29.1, 29.1], color=col, lw=2.4))
    T(x, 27.6, nm, fontsize=15.0, fontweight="bold", color=col)
    T(x, 26.2, q, fontsize=12.3, style="italic", color="#3d4852")
    T(x, 24.8, desc, fontsize=11.9, va="top")
    if arrow:
        T(x, 23.5, arrow, fontsize=11.9, va="top", fontweight="bold")
...
T(3, 20.4, "E M O T I O N A L   R E P R E S E N T A T I O N  —  "
  ...
  fontsize=11.2, fontweight="bold", color=GREY)
T(3, 18.5, "Emotional Representation", fontsize=15.0, fontweight="bold", color=GREY)
T(3, 17.2, '"How do I feel about it?"', fontsize=12.3, style="italic", color=GREY)
T(3, 16.0, "Emotional processing  →  no topic", fontsize=12.1,
...
T(26, 18.4, "Emotional processing did not emerge as an independent topic. Emotional "
            ...
            "(D1-3),", fontsize=11.4, va="top", color=GREY)
T(26, 17.1, "decisional distress (D2-5), diagnostic anxiety and coping (D3-5), ablation "
            ...
            "(D5-5),", fontsize=11.4, va="top", color=GREY)
T(26, 15.8, "consistent with the CSM dual-pathway hypothesis.", fontsize=11.4, va="top", color=GREY)

# ══════════════════════════════════════════════════════════ note
note = (
    "Note. Topics derived from LDA ($\\it{k}$ = 5, batch variational inference, random_state = 42) on 1,916 patient-authored posts across Xiaohongshu, Weibo, Zhihu and Douyin; the model is fully deterministic and\n"
    "reproduces from the deposited scripts. Topic labels were assigned by two researchers who independently inspected the topic–word distributions and representative posts and reconciled the result through\n"
    "discussion. Categories of patient concern come from directed content analysis of high-loading posts read during the analysis phase (16 institutional posts excluded; the coding set does not correspond exactly\n"
    "to the 1,257 posts above a dominant-topic probability of 0.60 in the final model). Comment sentiment is based on 12,380 topic-linked comments, each inheriting its parent post’s dominant topic. Sentiment was\n"
    "classified with the optimised domain-specific lexicon (80 terms; Supplemental Table S4) and is valid at group level only. CSM mapping was performed independently by two reviewers (100% agreement). Cause did\n"
    "not correspond to any topic and appears only at category level (D3-3 aetiology and risk factor awareness). Category percentages give the proportion of coded posts within each domain to which a category was\n"
    "applied and derive from the two-coder consensus coding.")
T(3, 12.4, note, fontsize=9.0, va="top", color=GREY, linespacing=1.2)
